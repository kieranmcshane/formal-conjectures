...
theorem lower_bound_test
  ...
  have hL_pos : 0 < L := by
    have h_lt_one : ε + r ≤ 1 := by
      have : ε₀_node3 ≤ 1 := by
        unfold ε₀_node3
        exact Real.exp_le_one_iff.mpr (by norm_num)
      exact le_trans hεr_le_ε₀ this
    have h_log_neg : Real.log (ε + r) < 0 := Real.log_neg hεr_pos (by linarith)
  ...
  have h_half_le : Real.exp (-2 * cLower_node3 * L^3) ≤ Real.exp (-cUpper_node3 * L^3) * (1 / 2 : ℝ) := by
    have h_bound : -2 * cLower_node3 * L^3 ≤ -cUpper_node3 * L^3 + Real.log (1 / 2 : ℝ) := by
      ...
      have h1 : 2 * cLower_node3 = 1 / 50000 := by unfold cLower_node3; norm_num
      have h2 : cUpper_node3 = 1 / 200000 := by unfold cUpper_node3; norm_num
      have hL3_ge : (1000000 : ℝ) ≤ L^3 := by
        have h100 : (100 : ℝ) ≤ L := by
          have h_le : Real.log (ε + r) ≤ Real.log ε₀_node3 := Real.log_le_log hεr_pos hεr_le_ε₀
          have h_log_val : Real.log ε₀_node3 = -100 := by unfold ε₀_node3; rw [Real.log_exp]
          rw [h_log_val] at h_le
          have h_neg : Real.log (ε + r) < 0 := by linarith
          rw [hL_def, abs_of_neg h_neg]
          linarith
        nlinarith
      rw [h1, h2]
      linarith
    have h_exp : Real.exp (-2 * cLower_node3 * L^3) ≤ Real.exp (-cUpper_node3 * L^3 + Real.log (1 / 2 : ℝ)) := Real.exp_le_exp.mpr h_bound
    have h_split_exp : Real.exp (-cUpper_node3 * L^3 + Real.log (1 / 2 : ℝ)) = Real.exp (-cUpper_node3 * L^3) * Real.exp (Real.log (1 / 2 : ℝ)) := Real.exp_add _ _
    rw [h_split_exp] at h_exp
    have h_exp_log : Real.exp (Real.log (1 / 2 : ℝ)) = 1 / 2 := Real.exp_log (by norm_num)
    rw [h_exp_log] at h_exp
    exact h_exp

  calc Real.exp (-2 * cLower_node3 * L^3)
      ≤ Real.exp (-cUpper_node3 * L^3) * (1 / 2 : ℝ) := h_half_le
    _ ≤ (∏ p ∈ R, P.block_smallball p ε) * (∏ p ∈ F, P.block_smallball p ε) := h_combine
    _ = ∏ p, P.block_smallball p ε := h_split.symm
