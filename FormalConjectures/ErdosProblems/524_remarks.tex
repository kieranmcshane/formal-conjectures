...
\begin{itemize}
  \item \textbf{Commit \texttt{c6e3cba}.} Added field
  ...
  \item \textbf{Commit \texttt{68d7240}.} Added
    \texttt{Helpers/CubicSubseqAsymptotics.lean} (351 lines, sorry-free):
    six theorems on the asymptotics of $n_m = \lfloor e^{m^3}\rfloor$
    needed for the Chojecki assembly. Introduced this \texttt{524\_remarks.tex}
    memo.
  \item \textbf{Commit \texttt{6e7bc7e}.} Refactored the big
  ...
  \item \textbf{Commit \texttt{315e8e1}.} Closed sorries \#2,
...
\paragraph{Outstanding.} The \texttt{chojecki\_sparse\_lower\_envelope\_proof}
theorem is still \texttt{sorry}; closing it requires the Borel–Cantelli +
block-independence + truncation assembly (see Remark 3 on nested
prefixes). An in-progress effort to replace the
\texttt{endpoint\_reparametrization} axiom with an actual theorem proof
exists in the workspace (file
\texttt{Erdos524/EndpointReparametrization.lean}) but has ~6 cascading
algebraic errors in its \texttt{biSup} manipulations; not yet committed.
