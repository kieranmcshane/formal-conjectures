...
theorem lower_bound_test
    ...
    (hεr_le_one : ε + r ≤ 1) (hεr_pos : 0 < ε + r)
  ...
  have h_rel_prod : (∏ p ∈ R, Real.exp (-cUpper_node3 * L^3 / m)) ≤ ∏ p ∈ R, P.block_smallball p ε := by
    ...
    · intro p hp
      ...
      exact P.relevant_block_bound p ε L hε hL_pos hp
  ...
  have h_R_card_le_m : (R.card : ℝ) ≤ (m : ℝ) := by
    have : R.card ≤ Fintype.card (Fin m) := card_le_univ _
    have h_fin : Fintype.card (Fin m) = m := Fintype.card_fin m
    rw [h_fin] at this
  ...
  have h_exp_mono : Real.exp (-cUpper_node3 * L^3) ≤ Real.exp (-cUpper_node3 * L^3 / m * R.card) := by
    ...
    rw [this]
    have h_c_L_nn : 0 ≤ cUpper_node3 * L^3 := by
      have h1 : 0 < cUpper_node3 := by unfold cUpper_node3; norm_num
